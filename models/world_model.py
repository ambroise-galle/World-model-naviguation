...
class LidarEncoder(nn.Module):
    ...
    """
    def __init__(self, num_rays=360, embed_dim=256):
        super().__init__()
        # Entrée: (Batch, 1, 360)
...
class MapDecoder(nn.Module):
    ...
    """
    def __init__(self, embed_dim=256, num_classes=10, map_size=64):
        super().__init__()
        ...
        self.init_size = 8
        self.fc = nn.Linear(embed_dim, 256 * self.init_size * self.init_size)
    ...
    def forward(self, x):
        # x: (Batch, embed_dim)
        x = F.relu(self.fc(x))
        x = x.view(-1, 256, self.init_size, self.init_size) # (Batch, 256, 8, 8)
        
        x = F.relu(self.deconv1(x)) # (Batch, 128, 16, 16)
        x = F.relu(self.deconv2(x)) # (Batch, 64, 32, 32)
...
class WorldModelAutoEncoder(nn.Module):
    ...
    """
    def __init__(self, num_rays=360, embed_dim=256, num_classes=10, map_size=64):
        super().__init__()
